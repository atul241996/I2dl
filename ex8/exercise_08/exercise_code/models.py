...
class Encoder(nn.Module):
    ...
    def __init__(self, hparams, input_size=28 * 28, latent_dim=20):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(self.input_size, self.hparams["encoder_hidden_size"]),
            nn.ReLU(),
            nn.Linear(self.hparams["encoder_hidden_size"], self.latent_dim)
        )
        pass
...
class Decoder(nn.Module):
    ...
    def __init__(self, hparams, latent_dim=20, output_size=28 * 28):
        ...
        self.decoder = nn.Sequential(
            nn.Linear(latent_dim, self.hparams["encoder_hidden_size"]),
            nn.ReLU(),
            nn.Linear(self.hparams["encoder_hidden_size"], output_size)
        )
...
class Classifier(pl.LightningModule):
    ...
    def __init__(self, hparams, encoder, train_set=None, val_set=None, test_set=None):
        ...
        self.model = nn.Sequential(
            nn.Linear(self.encoder.latent_dim, self.hparams["classifier_hidden_size"]),
            nn.ReLU(),
            nn.Linear(self.hparams["classifier_hidden_size"], 10)
        )
