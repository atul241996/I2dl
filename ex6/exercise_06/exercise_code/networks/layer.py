...
class LeakyRelu:
    ...
    def backward(self, dout, cache):
        ...
        dx = dout*np.where(cache<=0,self.slope,1)
        pass
...
class Tanh:
    ...
    def backward(self, dout, cache):
        ...
        dx = dout *(4*np.exp(-2*cache)/((1+np.exp(-2*cache))*(1+np.exp(-2*cache))))
        #def sigmoid(x):    return 1.0/(1.0 + np.exp(-x))def sigmoid_prime(x):return sigmoid(x)*(1.0-sigmoid(x))def tanh(x):    return 
        #np.tanh(x)def tanh_prime(x):return 1.0 - x**2
